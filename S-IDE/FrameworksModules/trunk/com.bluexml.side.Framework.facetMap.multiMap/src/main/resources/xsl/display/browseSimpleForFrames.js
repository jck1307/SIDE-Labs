...
function update_facets(facetPara, communityPara){
	var el_1 = parent.frames[0].document.getElementById('update_facets');
	if(el_1){
		parent.frames[0].document.location.replace('http://'+getHost()+'/facetmap/browseSimple-update.jsp?'+facetPara+communityPara);
	}
	else{
...
function setup(facetPara, communityPara){
	...
	if(el_1){
		parent.frames[0].document.location.replace('http://'+getHost()+'/facetmap/servletParameters.jsp?'+facetPara+communityPara);
	}
	else{
		//alert('Erreur3');
	}
