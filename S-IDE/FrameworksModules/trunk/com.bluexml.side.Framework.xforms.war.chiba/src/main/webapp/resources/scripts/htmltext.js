...
var _awaitingSets = new Array();

function _styledTextareaSetInnerHTMLWaiting(awaitingSetIndex) {
	var editorType = typeof FCKeditorAPI;
	if (editorType=="undefined") {
		setTimeout("_styledTextareaSetInnerHTMLWaiting(" + awaitingSetIndex + ")", 1000);
	} else {
		var awaitingSet = _awaitingSets[awaitingSetIndex];
		_styledTextareaSetInnerHTML(awaitingSet[0], awaitingSet[1]);
	}	 
}
...
function _styledTextareaSetInnerHTML (textarea, value) {
	var editorType = typeof FCKeditorAPI;
	if (editorType=="undefined") {
		var awaitingSet = new Array();
		awaitingSet.push(textarea, value);
	    _awaitingSets.push(awaitingSet);
	    var awaitingSetIndex = _awaitingSets.length - 1;
		setTimeout("_styledTextareaSetInnerHTMLWaiting(" + awaitingSetIndex + ")", 1000);
	} else {
