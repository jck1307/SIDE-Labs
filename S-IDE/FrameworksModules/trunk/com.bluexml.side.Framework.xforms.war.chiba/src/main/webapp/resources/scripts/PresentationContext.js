...
PresentationContext.prototype.handleHelperChanged = function(parentId, type, value) {
    ...
    switch (type) {
        ...
        case "alert":
            ...
            return;
        case "value":
            PresentationContext._setControlValue(parentId, value, type);
            return;
    }
};
...
PresentationContext._setControlValue = function(targetId, value, type) {
    ...
    switch (control.nodeName.toLowerCase()) {
        ...
        // <xf:output mediatype="text/html"/>
            ...
            if (type == "date" || type == "xsd:date") {
            	var dateWidget = dojo.widget.getWidgetById(targetId + "-value");
            	if (dateWidget) {
	            	var dateValue = dojo.date.fromRfc3339(value);
	            	if(!dateValue){
	            	 dateValue = new Date(value); 
	            	}
	            	dateWidget.setDate(dateValue);
            	}
				break;
            }
            if (type == "time" || type == "xsd:time") {
            	var timeWidget = dojo.widget.getWidgetById(targetId + "-value");
            	timeWidget.inputNode.value = value;
            	timeWidget.onInputChange();
				break;
            }
        ...
        if (generatedId) {
            ...
            if (element.parentNode)
            {
                ...
                if (clonedElement)
                {
                    ...
                    PresentationContext._patchGeneratedIds(clonedElement, id, generatedId);
                }
            }
...
//                dojo.debug("applying '" + generatedFor + "' for '" + id + "'");
            ...
            PresentationContext._patchGeneratedIds(element, id, generatedId);
        }
    }
...
PresentationContext._patchGeneratedIds = function(element, id, generatedId) {
    var onchangeEvent = dojo.html.getAttribute(element, 'onchange');
    if (onchangeEvent) {
        var reg = new RegExp("(" + id + ")", "g");
    	onchangeEvent = onchangeEvent.replace(reg, generatedId);
        element.onchange = function() { 
         eval(onchangeEvent); 
        };
    	//element.setAttribute('onchange', onchangeEvent);
    }
};
