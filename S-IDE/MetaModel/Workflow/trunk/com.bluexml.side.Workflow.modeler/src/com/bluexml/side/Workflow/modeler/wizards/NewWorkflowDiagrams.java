 ...
 * @generated
 */
public class NewWorkflowDiagrams extends Wizard implements INewWizard {
	/**
	 * @generated
	 */
	...
	public void init(IWorkbench workbench, IStructuredSelection sel) {
		...
		setDefaultPageImageDescriptor(WorkflowImageRegistry
				.getImageDescriptor("NEW_PAGE_WZD"));
		setDialogSettings(WorkflowPlugin.getDefault().getDialogSettings());
		setWindowTitle("Create new Workflow diagrams");
	}
		...
		if (diagPage.isPageComplete()) {
			if (diagPage.isNewModel()) {
				...
				if (createdFile != null && result) {
					...
					try {
						IDE.openEditor(WorkflowPlugin.getDefault()
								.getWorkbench().getActiveWorkbenchWindow()
								.getActivePage(), createdFile, true);
					} catch (PartInitException pie) {
						WorkflowPlugin.log(pie);
					}
				}
			} else {
	...
	public void addPages() {
		diagPage = new WorkflowDiagramsPage("New Workflow modeler Diagram",
				selection);
		diagPage.setTitle("Workflow Diagrams");
	...
	private boolean createModelFile() {
		try {
			...
			WorkflowPlugin.log(ce);
			WorkflowPlugin.displayDialog(null,
