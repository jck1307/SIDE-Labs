...
import com.bluexml.side.Requirements.modeler.RequirementsImageRegistry;
 ...
 * @generated
 */
public class NewRequirementsDiagrams extends Wizard implements INewWizard {
	/**
	 * @generated
	 */
	...
	public void init(IWorkbench workbench, IStructuredSelection sel) {
		...
		setDefaultPageImageDescriptor(RequirementsImageRegistry
				.getImageDescriptor("NEW_PAGE_WZD"));
		setDialogSettings(RequirementsPlugin.getDefault().getDialogSettings());
		setWindowTitle("Create new Requirements diagrams");
	}
		...
		if (diagPage.isPageComplete()) {
			if (diagPage.isNewModel()) {
				...
				if (createdFile != null && result) {
					...
					try {
						IDE.openEditor(RequirementsPlugin.getDefault()
								.getWorkbench().getActiveWorkbenchWindow()
								.getActivePage(), createdFile, true);
					} catch (PartInitException pie) {
						RequirementsPlugin.log(pie);
					}
				}
			} else {
	...
	public void addPages() {
		diagPage = new RequirementsDiagramsPage(
				"New Requirements modeler Diagram", selection);
		diagPage.setTitle("Requirements Diagrams");
	...
	private boolean createModelFile() {
		try {
			...
			RequirementsPlugin.log(ce);
			RequirementsPlugin.displayDialog(null,
	...
	private boolean createDiagramFile() {
		try {
			...
			template.addVariable("escapedName", URI.encodeFragment(diagPage
					.getModelName(), false));
