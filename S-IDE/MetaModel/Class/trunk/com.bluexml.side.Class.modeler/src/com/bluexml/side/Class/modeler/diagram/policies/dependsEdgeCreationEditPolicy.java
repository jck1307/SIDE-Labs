...
import com.bluexml.side.Class.modeler.diagram.commands.dependsEdgeCreationCommand;

/**
 * depends edge creation
 *
 * @generated
 */
public class dependsEdgeCreationEditPolicy extends AbstractEdgeCreationEditPolicy {
	/**
	...
	protected CreateTypedEdgeCommand createCommand(EditDomain domain, GraphEdge edge, GraphElement source) {
		return new dependsEdgeCreationCommand(domain, edge, source);
	}
	...
	protected boolean checkEdge(GraphEdge edge) {
		if (edge.getSemanticModel() instanceof SimpleSemanticModelElement) {
			return CdSimpleObjectConstants.SIMPLE_OBJECT_DEPENDS.equals(((SimpleSemanticModelElement) edge.getSemanticModel()).getTypeInfo());
		}
		return false;
	}
	...
	protected boolean checkSource(GraphElement source) {
		...
		if (object instanceof com.bluexml.side.clazz.Enumeration) {
			return true;
		}
		return false;
	}
	...
	protected boolean checkTargetForSource(GraphElement source, GraphElement target) {
		...
		if (sourceObject instanceof com.bluexml.side.clazz.Enumeration && targetObject instanceof com.bluexml.side.clazz.Enumeration) {
	...
	protected boolean checkCommand(Command command) {
		return command instanceof dependsEdgeCreationCommand;
	}
	...
	protected SourceTargetData getSourceTargetData(GraphElement source, GraphElement target) {
		...
		if (sourceObject instanceof com.bluexml.side.clazz.Enumeration && targetObject instanceof com.bluexml.side.clazz.Enumeration) {
			return new SourceTargetData(false, false, SourceTargetData.NONE, null, null, null, null, null, "depends", null, null);
		}
		return null;
	}
