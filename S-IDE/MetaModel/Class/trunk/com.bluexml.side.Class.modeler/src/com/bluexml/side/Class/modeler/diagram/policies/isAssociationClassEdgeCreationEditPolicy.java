...
import com.bluexml.side.Class.modeler.diagram.commands.isAssociationClassEdgeCreationCommand;

/**
 * isAssociationClass edge creation
 *
 * @generated
 */
public class isAssociationClassEdgeCreationEditPolicy extends AbstractEdgeCreationEditPolicy {
	/**
	...
	protected CreateTypedEdgeCommand createCommand(EditDomain domain, GraphEdge edge, GraphElement source) {
		return new isAssociationClassEdgeCreationCommand(domain, edge, source);
	}
	...
	protected boolean checkEdge(GraphEdge edge) {
		if (edge.getSemanticModel() instanceof SimpleSemanticModelElement) {
			return CdSimpleObjectConstants.SIMPLE_OBJECT_ISASSOCIATIONCLASS.equals(((SimpleSemanticModelElement) edge.getSemanticModel()).getTypeInfo());
		}
		return false;
	}
	...
	protected boolean checkCommand(Command command) {
		return command instanceof isAssociationClassEdgeCreationCommand;
	}
