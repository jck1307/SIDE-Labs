...
<%getProperty("alf.paths.extension.side.webscripts")%>/<%getFolder()%>/<%getQualifiedName().replaceAll("_","/")%>/<%getQualifiedName()%>.post.json.ftl
	...
	"records":[
						...
						<#list records as child>
							...
							<#if ((time==0) || (dateCompare(child.properties["cm:modified"], date, time) == 1) || (dateCompare(child.properties["cm:created"], date, time) == 1))>
								<#if minBound<=index>
								<#include "json/<%getQualifiedName()%>.ftl">
								</#if>
