...
(function()
{
   ...
   YAHOO.lang.augmentObject(Alfresco.DocumentList.prototype,
   {
      ...
      onReady: function DL_onReady()
      {
         ...
         // SIDE sorting feature
         this.widgets.fileSort = Alfresco.util.createYUIButton(this, "sort-button", this.onSort,
         {
            type: "menu", 
            menu: "sort-menu"
         });
               ...
               else
               {
                  ...
                  if (record.custom && record.custom.isWorkingCopy)
                  {
                     ...
                     */
                     ...
                     for (var x in customProp) {
                    	 ...
                    	 desc += '<span class="item"><em>' + me._msg(i18nKey)+ '</em> ' + customProp[x] + '</span>';                    	 
                     }
                     desc += '</div>';
                  }
                  else
                  {
                     ...
                     */
                     ...
                     for (var x in customProp) {
                    	 var i18nKey = x.replace(/.*:/,"");
                    	 desc += '<span class="item"><em>' + scope.msg(i18nKey)+ '</em> ' + customProp[x] + '</span>';                    	 
                     }
                     desc += '</div>';
	 ...
	 *
	 */
	onSort: function DL_onSort(sType, aArgs, p_obj) {
		var path = p_obj.currentPath;
		var index = aArgs[1].index;
		var key =aArgs[0].originalTarget.className.replace("sort-","");
		this.sorting=key;
      ...
      _buildDocListParams: function DL__buildDocListParams(p_obj)
      {
         ...
         // SIDE sorting
         if (this.sorting) {
         	params += "&sorting=" + encodeURIComponent(this.sorting);
         }
         return params;
      },
